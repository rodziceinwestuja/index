    ...
    <div className="animate-fade-in max-w-3xl mx-auto text-center">
      ...
      <div className="mb-12 max-w-2xl mx-auto">
          ...
          <p className="text-sm text-gray-600 leading-relaxed">
              Oprocentowanie Obligacji Rodzinnych w kolejnych latach to suma inflacji i marży (marża jest wyższa niż dla standardowych obligacji). <span className="text-blue-600 font-bold">Marża to Twój gwarantowany, realny zysk nad inflację w długim terminie.</span>
          </p>
      </div>
      
      <div className="grid md:grid-cols-2 gap-6 mb-12 text-left">
        <div onClick={openProviderModal} className={cardStyle}>
            <i className="fas fa-baby absolute -bottom-5 -right-5 text-8xl text-blue-50 opacity-50 transition-transform duration-500"></i>
          ...
          <a href={BOND_DATA.ROS.link} onClick={(e) => e.stopPropagation()} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-blue-400 hover:text-blue-600 mb-2 inline-flex items-center gap-1 relative z-20 transition-colors">
              <i className="fas fa-info-circle"></i> Szczegóły oferty
          </a>

          <p className="text-xs text-gray-400 relative z-10 mb-6">Kapitalizacja roczna. Idealne na krótszy horyzont.</p>
          ...
          <div className="mt-auto relative z-10 bg-blue-50 text-blue-600 font-bold px-6 py-3 rounded-full text-sm group-hover:bg-blue-100 transition-colors mx-auto flex items-center gap-2 w-fit">
              Zobacz gdzie kupić <i className="fas fa-chevron-right text-xs"></i>
          </div>
        </div>

        <div onClick={openProviderModal} className={cardStyle}>
            <i className="fas fa-graduation-cap absolute -bottom-5 -right-5 text-8xl text-blue-50 opacity-50 transition-transform duration-500"></i>
          ...
          <a href={BOND_DATA.ROD.link} onClick={(e) => e.stopPropagation()} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-blue-400 hover:text-blue-600 mb-2 inline-flex items-center gap-1 relative z-20 transition-colors">
              <i className="fas fa-info-circle"></i> Szczegóły oferty
          </a>

          <p className="text-xs text-gray-400 relative z-10 mb-6">Kapitalizacja roczna. Najwyższy zysk na start w dorosłość.</p>
