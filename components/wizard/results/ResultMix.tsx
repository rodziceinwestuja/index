...
const ResultMix: React.FC = () => {
  ...
  const [safetySlider, setSafetySlider] = useState(40);
  ...
  const getCardStyle = (isActive: boolean, variant: 'emerald' | 'blue' | 'amber') => {
    const colorClasses = {
      emerald: 'border-emerald-100 shadow-emerald-100/40',
      blue: 'border-blue-100 shadow-blue-100/40',
      amber: 'border-amber-100 shadow-amber-100/40',
    };

    if (isActive) {
      return `w-full text-left bg-white p-6 rounded-3xl border shadow-lg transition-all duration-300 ease-out flex flex-col h-full relative overflow-hidden opacity-100 scale-100 ${colorClasses[variant]}`;
    }
    ...
    <div className="animate-fade-in max-w-6xl mx-auto pb-20">
      ...
      <div className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-100 shadow-xl shadow-gray-100/50 mb-8 relative z-20 max-w-4xl mx-auto">
        <div className="flex justify-center mb-8">
          <div className="bg-gray-50 rounded-2xl p-3 md:p-4 border border-gray-100 flex flex-col items-center min-w-[200px]">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Kwota do podziału</label>
        ...
        <div className="flex justify-between items-end mb-6 select-none relative">
          <div className={`text-left group cursor-default w-1/3 transition-opacity duration-500 ${stockPct === 0 ? 'opacity-30' : 'opacity-100'}`}>
            <div className="mb-1">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Akcje / ETF</span>
            </div>
          ...
          <div className={`text-center absolute left-1/2 -translate-x-1/2 bottom-0 animate-fade-in w-1/3 flex flex-col items-center justify-end h-full pb-1 transition-all duration-500 ${includeGold ? 'opacity-100 transform-none' : 'opacity-30 grayscale'}`}>
            <div className="hidden sm:flex items-center justify-center gap-2 mb-1">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Kruszce</span>
            </div>
          ...
          <div className={`text-right group cursor-default w-1/3 ml-auto transition-opacity duration-500 ${bondPct === 0 ? 'opacity-30' : 'opacity-100'}`}>
            <div className="mb-1">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Obligacje</span>
            </div>
        ...
        <div className="mb-8 px-1">
          ...
          />
          <div className="flex justify-between text-xs font-bold text-gray-300 mt-2 uppercase tracking-widest px-1">
            <span>Ryzykowny</span>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between border-t border-gray-100 pt-6">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <span className="text-xs text-gray-400">Dodatek dywersyfikujący</span>
            </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-stretch justify-center">
        <div className="flex-1 min-w-0">
          <div className={getCardStyle(stockPct > 0, 'emerald')}>
        ...
        <div className="flex-1 min-w-0">
          <div className={getCardStyle(bondPct > 0, 'blue')}>
        ...
        <div className="flex-1 min-w-0">
          <div className={getCardStyle(includeGold, 'amber')}>
      ...
      <div className="mt-8 p-6 bg-gray-50 rounded-3xl border border-gray-100 text-sm text-gray-600 flex items-start gap-4 italic">
